...
class OutLayer3(tf.keras.layers.Layer):
    """`OutLayer` updates the network output with a `FFLayer` layer, where the
    `out_units` controls the dimension of outputs. In addition to the `FFLayer`
    specified by `n_nodes`, the `OutLayer` has one additional linear biasless
    layer that scales the outputs, specified by `out_units`.
    ...
    def call(self, tensors):
        ...
        ind_1, p1, p3, prev_output = tensors
        p3 = self.ff_layer(p3)
        output = self.out_units(p3) + prev_output
        return output
...
class GCBlock(tf.keras.layers.Layer):
    def __init__(self, weighted: bool, pp_nodes, pi_nodes, ii_nodes, **kwargs):
        ...
        self.pix_layer = PIXLayer(pp_nodes, weighted=weighted, **kwargs)
...
class PiNet2_norm_p3_i(tf.keras.Model):
    ...
    def __init__(
        ...
        """
        super(PiNet2_norm_p3_i, self).__init__()
        ...
        self.pout1_layers = [OutLayer(out_nodes, out_units) for i in range(depth)]
        self.pout3_layers = [OutLayer3(out_nodes, out_units) for i in range(depth)]
    ...
    def call(self, tensors):
        ...
        if self.iout_layers is not None:
            ...
            for i in range(self.depth):
                ...
                pout1 = self.pout1_layers[i]([tensors["ind_1"], p1, p3, pout1])
                pout3 = self.pout3_layers[i]([tensors["ind_1"], p1, p3, pout3])
            ...
            pout1 = self.ann_output([tensors["ind_1"], pout1])
            pout3 = tf.squeeze(pout3, axis=-1)

            return pout1, pout3, iout3
