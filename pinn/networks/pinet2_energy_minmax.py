...
class GCBlock(tf.keras.layers.Layer):
    ...
    def call(self, tensors):
        ...
        #p3_dot = self.dot_layer(p3) 
        #p1t1 = p3_dot / tf.math.reduce_max(p3_dot) + p1
...
class PiNet2_energy_minmax(tf.keras.Model):
    ...
    def __init__(
        ...
        """
        super(PiNet2_energy_minmax, self).__init__()
    ...
    def call(self, tensors):
        ...
        for i in range(self.depth):
            ...
            p1 = (p1 - tf.reduce_min(p1)) / (tf.reduce_max(p1) - tf.reduce_min(p1))
            p3 = (p3 - tf.reduce_min(p3)) / (tf.reduce_max(p3) - tf.reduce_min(p3))
            ...
            tensorP1Min = tf.reduce_min(tensors["p1"])
            tensors["p1"] = (tensors["p1"] - tensorP1Min) / (tf.reduce_max(tensors["p1"]) - tensorP1Min)
            tensorP3Min = tf.reduce_min(tensors["p3"])
            tensors["p3"] = (tensors["p3"] - tensorP3Min) / (tf.reduce_max(tensors["p3"]) - tensorP3Min)
